...
interface MapAlertInterface {
    coordinates: { sensor: string, top: number, left: number }[];
    isVisible: boolean;
    onclose: () => void;
}
        ...
        <>
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-40 py-20">
                <div className="bg-white p-6 rounded-lg shadow-lg w-full h-full relative">
                    ...
                    <h2 className="text-xl font-semibold mb-4">Map Alert</h2>
                    <div className="h-[calc(100%-28px)] relative">
                        ...
                        {coordinates.map((coord, index) => (
                            <span
                                key={index}
                                title={coord.sensor} // Tooltip text
                                className="absolute w-4 h-4 rounded-full bg-red-500 opacity-75 animate-ping"
